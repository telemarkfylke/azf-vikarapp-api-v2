...
const validatePost = (requestBody: SubstitutionRequest[], requestor: Requestor, logPrefix: string): SubstitutionRequest[] => {
  const deduped = requestBody.filter((item, index, self) => index === self.findIndex((t) => t.teacherUpn === item.teacherUpn && t.teamId === item.teamId));
...
const handleGet = async (request: HttpRequest, context: InvocationContext, requestor: Requestor): Promise<HttpResponseInit> => {
  ...
  if (teacherUpn) clauses.push({ teacherUpn });
  if (substituteUpn) clauses.push({ substituteUpn });
  if (years.length > 0) {
    const $or = years.map((y) => {
  ...
  try {
    ...
    const substitutions = await mongoClient.db(mongoDB.DB_NAME).collection(mongoDB.SUBSTITUTIONS_COLLECTION).find(filter).sort({ expirationTimestamp: -1 }).toArray();
    ...
    return { status: 500, jsonBody: errorBody(error) };
  }
};
...
const handlePost = async (request: HttpRequest, context: InvocationContext, requestor: Requestor, requestBody: SubstitutionRequest[]): Promise<HttpResponseInit> => {
  const logPrefix = "substitutions - post";
  const mongoClient = await getMongoClient();
  ...
  for (const upn of uniqueSubstituteUpns) {
    const substitute = (await getUser(upn)) as EnrichedSubstitute | null;
    if (!substitute) {
    ...
    const existingSubstitutions = await mongoClient.db(mongoDB.DB_NAME).collection(mongoDB.SUBSTITUTIONS_COLLECTION).find({ substituteId: substitute.id }).toArray();
  ...
  for (const upn of uniqueTeacherUpns) {
    const teacher = (await getUser(upn)) as GraphUser | null;
    if (!teacher?.id) {
    ...
    teachers.push({ ...teacher, owned: ownedResources });
  }

  const expirationTimestamp = new Date(new Date().setHours(1, 0, 0, 0) + 3 * 24 * 60 * 60 * 1000);
  const newSubstitutions: Substitution[] = [];
  const renewedSubstitutions: { extendedSubstitution: SubstitutionRequest; _id: ObjectId; expirationTimestamp: Date }[] = [];
  const renewedExpiredSubstitutions: { expiredSubstitution: SubstitutionRequest; _id: ObjectId; expirationTimestamp: Date }[] = [];
  ...
  try {
    for (const substitution of requestBody) {
      ...
      if (activeSubstitution) {
        ...
        renewedSubstitutions.push({ extendedSubstitution: { ...substitution }, _id: activeSubstitution._id, expirationTimestamp });
        ...
        renewedExpiredSubstitutions.push({ expiredSubstitution: { ...substitution }, _id: expiredSubstitution._id, expirationTimestamp });
      } else {
    ...
    for (const newSubstitution of newSubstitutions) {
      try {
        ...
        try {
          ...
          await logToDB("error", error, request, context, requestor);
          return { status: 404, jsonBody: errorBody(error) };
        }
      } catch (error) {
...
const handlePut = async (request: HttpRequest, context: InvocationContext, requestor: Requestor, ids: string[]): Promise<HttpResponseInit> => {
  const logPrefix = "substitutions - put";
  const mongoClient = await getMongoClient();
  ...
  try {
    ...
    const substitutions = (await mongoClient
      .db(mongoDB.DB_NAME)
      .collection(mongoDB.SUBSTITUTIONS_COLLECTION)
    ...
    const response = await deactivateSubstitutions(undefined, substitutions, request, context);
...
const handler = async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
  ...
  const logPrefix = "substitutions - validate";
  ...
  if (request.method === "POST") {
    ...
    const requestBody = validatePost(rawBody as SubstitutionRequest[], requestor, logPrefix);
    return handlePost(request, context, requestor, requestBody);
  }

  if (request.method === "PUT") {
    const rawBody = (await request.json()) as unknown;
    const ids = validatePut(rawBody, requestor, logPrefix);
